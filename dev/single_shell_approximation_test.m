...
sTrue = source2d_variable_spacing([peakPos(2),ones(1,1)*1.2*r],[0,-1],n0);
sLinear = source2d_variable_spacing([peakPos(2),ones(1,1)*1.2*r],[0,-1],n0);
ds = 10^3;
...
plotCircle(r,2*pi,2)
plotLine([-1.2*r 0],[1.2*r 0],'k',2)
plotLine([0 -1.2*r],[0 1.2*r],'k',2)
sTrue.plotTrace(2,'r')
sTrue.plotRay_back(1,0,'--r',2);
sLinear.plotTrace(2,'g')
sLinear.plotRay_back(1,0,'--g',2);


% Now tracing the testsource using the alogrithm in the aproximation script

sSliceTest = source2d_variable_spacing([peakPos(2),ones(1,1)*1.2*r],[0,-1],n0);
slice_trace_test(sSliceTest,ds,r,n0,kL,1)
sSliceTest.plotTrace(2,'b')
sSliceTest.plotRay_back(1,0,'--b',2);
% legend('True','Approximation','True-Linear')
...
nmax = getLinearGradient_CurvedTrace(ra,rb,n0,ns,sSliceTest.phase(1),sSliceTest.PT{1});
...
[phaseTot,~] = findTotPhase(pT,vT,peakPos,peaksDiff);
...
plot(peakPos,peaksDiff,'b')
plot(phaseDiff(:,1),phaseDiff(:,2),'r'),
