% In use
function n = homogenious_approximation_elliptical(source, C)
...
[ip0,ip1,~] = ellipseIntersect(shellR(1),shellR(1)*C.rMinor, source.P(1,:),source.V(1,:));
dv = (ip1-ip0);
...
for rayInd = 2:source.nRays
    ...
    for shellInd = 1:rayInd
        [ip0,ip1,~] = ellipseIntersect(shellR(shellInd),shellR(shellInd)*C.rMinor, source.P(rayInd,:),source.V(rayInd,:));
        dv = (ip1-ip0);
        d = sqrt(dv(1)^2+dv(2)^2); % distance traveled in shell
        dShells(shellInd) = d;
    end
