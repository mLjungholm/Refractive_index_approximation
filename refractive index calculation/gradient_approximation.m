% In use
function gradient_approximation(source, C, steps, varargin)
...
[~] = loopRay(rayInd);
    ...
    function stopflag = loopRay(rayInd)
        ...
        while iter < maxIter
            ...
            if iter == 1
                nLow = C.n(rayInd);
                nHigh = (projectedPhase - outerPhase)/inner_d + C.n(1);
                iter = iter + 1;
                continue
            end
    ...
    function [p1,v1] = intStep(p0,v0,shellInd)
        ...
        nr = C.getN(shellInd,r0);                     % Get refractive index in ray-point based on the current n-gradient
    ...
    function dP = phaseShift(r0,r1,shellInd)
        nr = C.getN(shellInd,(r0+r1)/2); % Get refractive index in ray-point based on the current n-gradient
